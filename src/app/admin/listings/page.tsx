...
function ListingsContent() {
  ...
  return (
    <div className="px-4 py-5 max-w-7xl">
      ...
      <div className="flex items-center justify-between gap-3 mb-4"><p className="text-sm text-muted">총 <b className="text-foreground">{listings.length}</b>건</p><select value={status} onChange={(event) => setStatus(event.target.value)} className="h-10 px-3 rounded-lg border border-border text-sm bg-background" aria-label="상품 상태 필터"><option value="">전체 상태</option><option value="PENDING">심사 대기</option><option value="ACTIVE">판매중</option><option value="REJECTED">반려</option><option value="SOLDOUT">품절</option><option value="DRAFT">작성중</option><option value="HIDDEN">비노출</option></select></div>

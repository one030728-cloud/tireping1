    ...
    return (
      <div className="flex justify-center px-4 py-16">
        <div className="card w-full max-w-lg p-8 text-center">
          <h1 className="text-xl font-extrabold">가입 신청이 접수되었습니다.</h1>
          <p className="text-sm text-muted leading-relaxed mt-3">
            본사 관리자가 사업자 정보를 확인한 뒤 판매자 계정을 승인합니다. 승인 완료 후 입력한 아이디로 로그인할 수 있습니다.
          </p>
  ...
  return (
    <div className="flex justify-center px-4 py-10">
      <form onSubmit={handleSubmit} className="card w-full max-w-3xl p-6 md:p-8">
        <div className="mb-6">
          <Link href="/login" className="text-sm text-muted hover:text-brand">← 로그인</Link>
          <h1 className="text-xl font-extrabold mt-3">판매자 가입 신청</h1>
        ...
        <section className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Input label="판매자 코드" value={form.code} onChange={(value) => setField("code", value)} placeholder="예: T30001" required />
          </div>
        </section>
        ...
        <section className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Input label="휴대전화" value={form.mobilePhone} onChange={(value) => setField("mobilePhone", value)} required />
            ...
            <Input label="사무실 전화" value={form.officePhone} onChange={(value) => setField("officePhone", value)} />
        ...
        <section className="mb-6">
          <h2 className="font-bold mb-3">배송 및 판매자 소개</h2>
          <div className="grid sm:grid-cols-2 gap-3">
            <Input label="기본 택배사" value={form.courier} onChange={(value) => setField("courier", value)} required />
            <Input label="판매자 위치" value={form.location} onChange={(value) => setField("location", value)} />
            <TextArea label="기본 배송 안내" value={form.shippingNote} onChange={(value) => setField("shippingNote", value)} />
            <TextArea label="판매자 소개" value={form.intro} onChange={(value) => setField("intro", value)} />
          </div>
        </section>
...
function Input({ label, value, onChange, type = "text", placeholder, required }: { label: string; value: string; onChange: (value: string) => void; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="flex flex-col gap-1.5 text-sm">
      ...
      <input type={type} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="seller-input" required={required} />
    </label>
  );
}
...
function TextArea({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return (
    <label className="flex flex-col gap-1.5 text-sm">
      ...
      <textarea value={value} onChange={(event) => onChange(event.target.value)} className="min-h-24 p-3 rounded-lg border border-border bg-surface resize-y focus:outline-none focus:border-brand" />
    </label>
  );
}
